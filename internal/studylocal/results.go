...
func validateResults(packageDirectory, authorizationDirectory, runDirectory string) (validatedRun, error) {
	...
	required := []string{
		"adapter-workspace", "authorization.json", "calls", "completion.json",
		"host-preflight.json", "model-manifest.json", "run-manifest.json", "runtime-manifest.json",
	}
...
func validateObservation(directory string, entry ScheduleEntry, pkg Package) (ExecutionReceipt, error) {
	...
	if err := strictDecode(files["receipt.json"], &receipt); err != nil {
		return ExecutionReceipt{}, err
	}
	...
	if receipt.Status != "adapter_error" {
		if err := strictDecode(files["adapter-stdout.bin"], &adapterResponse); err != nil {
