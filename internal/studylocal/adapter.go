...
func StartCommandAdapter(ctx context.Context, options CommandAdapterOptions) (Adapter, error) {
	...
	args := []string{
		"-p", SandboxPolicy, python, "-I", "-B", adapterPath, "serve",
	...
	go scanAdapterLines(stdout, lines)
	stderr := &boundedBuffer{limit: maxAdapterStderrBytes}
	go func() {
		_, _ = io.Copy(stderr, stderrPipe)
	}()
	session := &commandAdapter{command: command, stdin: stdin, lines: lines, stderr: stderr}
	...
	if err := strictDecode(line, &session.ready); err != nil {
		_ = session.forceStop()
		return nil, fmt.Errorf("adapter ready envelope: %w", err)
	}
	if session.ready.SchemaVersion != AdapterReadySchema || !session.ready.ModelLoaded ||
		session.ready.AdapterSHA256 != DigestBytes(adapterBytes) ||
		session.ready.ImplementationCommit != options.ImplementationCommit {
		_ = session.forceStop()
		return nil, fmt.Errorf("adapter ready identity mismatch")
	}
	return session, nil
}
...
func (adapter *commandAdapter) Generate(ctx context.Context, request AdapterRequest) (AdapterResponse, []byte, []byte, error) {
	...
	if err := strictDecode(rawEnvelope, &response); err != nil {
...
func scanAdapterLines(reader io.Reader, out chan<- adapterLine) {
	defer close(out)
	scanner := bufio.NewScanner(reader)
	scanner.Buffer(make([]byte, 64<<10), maxAdapterLineBytes)
	for scanner.Scan() {
		out <- adapterLine{data: append([]byte(nil), scanner.Bytes()...)}
	}
