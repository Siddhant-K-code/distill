...
var packageFiles = []string{
	"contexts.jsonl",
	"corpus.json",
	"local-control-result-schema-v2.json",
...
func BuildArtifacts(temporaryParent string) (map[string][]byte, Summary, error) {
	...
	files["local-control-result-schema-v2.json"] = resultSchema()
	...
	manifest := PackageManifest{
		...
		ScheduleSHA256: scheduleHash, ResultSchemaSHA256: DigestBytes(files["local-control-result-schema-v2.json"]),
...
func ValidatePackage(directory string) (Package, error) {
	...
	if err := strictJSONObjectFileDecode(files["protocol.json"], &protocol); err != nil {
	...
	if err := strictJSONFileDecode(files["source-registry.json"], &registry); err != nil ||
		fmt.Sprint(registry) != fmt.Sprint(corpus.SourceRegistry) {
		return Package{}, fmt.Errorf("source registry mismatch")
	}
	if !bytes.Equal(files["local-control-result-schema-v2.json"], resultSchema()) {
		return Package{}, fmt.Errorf("result schema drift")
	}
	var manifest PackageManifest
	if err := strictJSONObjectFileDecode(files["package-manifest.json"], &manifest); err != nil {
		return Package{}, err
	}
	expectedManifest := PackageManifest{
		...
		ResultSchemaSHA256: DigestBytes(files["local-control-result-schema-v2.json"]),
		SourceRegistryHash: DigestBytes(files["source-registry.json"]),
...
func resultSchema() []byte {
	...
	if err := json.Unmarshal([]byte(`{
	  ...
	  "$id": "https://github.com/Siddhant-K-code/distill/research/context-is-a-build-artifact/local-control-result-schema-v2.json",
	  "title": "Local context-control categorical model projection v2",
	  "type": "object",
