...
const AuthorizationAcknowledgement = "I authorize only the pinned local 4B v2 development/calibration schedule with zero network access, zero cloud spend, and no scaling control."
...
type ExecutionAuthorization struct {
	...
	AdapterCheckRequestSHA256    string    `json:"adapter_check_request_sha256"`
	AdapterCheckReceiptSHA256    string    `json:"adapter_check_receipt_sha256"`
	AdapterCheckDirectorySHA256  string    `json:"adapter_check_directory_sha256"`
	AdapterCheckExpiresAt        time.Time `json:"adapter_check_expires_at"`
...
func Authorize(ctx context.Context, options AuthorizeOptions) (ExecutionAuthorization, error) {
	...
	checkOptions := AdapterCheckOptions{
		PackageDirectory: options.PackageDirectory, ModelDirectory: options.ModelDirectory,
		ModelManifestPath: options.ModelManifestPath, RuntimePython: options.RuntimePython,
		...
		AdapterPath:             options.AdapterPath, RepositoryRoot: options.RepositoryRoot,
		MainRef: options.MainRef, TrustedImplementationCommit: options.TrustedImplementationCommit,
		TrustedImplementationTree: options.TrustedImplementationTree,
		RunOutputDirectory:        options.RunOutputDirectory, OutputDirectory: options.AdapterCheckDirectory,
	}
	checkRequest, checkReceipt, checkHost, err := LoadAndVerifyAdapterCheck(
		ctx, options.AdapterCheckDirectory, checkOptions, time.Now().UTC(),
	)
	if err != nil {
		return ExecutionAuthorization{}, fmt.Errorf("adapter check: %w", err)
	}
	checkFiles, err := readAdapterCheckFilesForAuthorization(options.AdapterCheckDirectory)
	if err != nil {
		return ExecutionAuthorization{}, err
	}
	...
	if host.OSVersion != checkHost.OSVersion || host.OSBuild != checkHost.OSBuild ||
		host.Architecture != checkHost.Architecture || host.Chip != checkHost.Chip ||
		host.PhysicalMemoryBytes != checkHost.PhysicalMemoryBytes ||
		host.PowerSource != checkHost.PowerSource ||
		host.SandboxExecAvailable != checkHost.SandboxExecAvailable {
		return ExecutionAuthorization{}, fmt.Errorf("host identity changed after adapter check")
	}
	...
	if !options.CreatedAt.Before(checkReceipt.ExpiresAt) {
		return ExecutionAuthorization{}, fmt.Errorf("adapter-check receipt expired before authorization")
	}
	...
	genesis := AttemptLedgerGenesis{
		...
		AdapterCheckReceiptSHA256: checkReceipt.ReceiptSHA256,
	}
	...
	authorization := ExecutionAuthorization{
		...
		AdapterCheckRequestSHA256:    checkRequest.RequestSHA256,
		AdapterCheckReceiptSHA256:    checkReceipt.ReceiptSHA256,
		AdapterCheckDirectorySHA256:  checkReceipt.AdapterCheckDirectoryHash,
		AdapterCheckExpiresAt:        checkReceipt.ExpiresAt,
		FramingConformanceSHA256:     checkReceipt.FramingConformanceSHA256,
	...
	files := map[string][]byte{
		"authorization.json": authorizationBytes, "adapter-check.json": checkFiles["adapter-check.json"],
	...
	if err := consumeAdapterCheck(options.AdapterCheckDirectory, checkRequest, checkReceipt, authorization); err != nil {
		return ExecutionAuthorization{}, err
	}
...
func LoadAuthorization(directory string) (ExecutionAuthorization, HostSnapshot, ModelManifest, RuntimeManifest, map[string][]byte, error) {
	...
	if err := strictJSONObjectFileDecode(files["authorization.json"], &authorization); err != nil {
		return ExecutionAuthorization{}, HostSnapshot{}, ModelManifest{}, RuntimeManifest{}, nil, err
	}
	...
	if authorization.SchemaVersion != SchemaVersion+"/execution-authorization" ||
		...
		authorization.AdapterCheckRequestSHA256 == "" ||
		authorization.AdapterCheckReceiptSHA256 == "" ||
		authorization.AdapterCheckDirectorySHA256 == "" ||
		authorization.AdapterCheckExpiresAt.IsZero() ||
		authorization.FramingConformanceSHA256 == "" ||
		authorization.MainRef != "origin/main" ||
		len(authorization.ImplementationCommit) != 40 || len(authorization.ImplementationTree) != 40 ||
		authorization.BuildPackage != "github.com/Siddhant-K-code/distill/cmd/distill-local-context-control-v2" ||
	...
	var checkReceipt AdapterCheckReceipt
	if err := strictJSONObjectFileDecode(files["adapter-check.json"], &checkReceipt); err != nil {
		return ExecutionAuthorization{}, HostSnapshot{}, ModelManifest{}, RuntimeManifest{}, nil, err
	}
	checkReceiptDigest, _ := DigestDomain("adapter-check-receipt", adapterCheckReceiptProjection(checkReceipt))
	if checkReceipt.ReceiptSHA256 != checkReceiptDigest ||
		checkReceipt.ReceiptSHA256 != authorization.AdapterCheckReceiptSHA256 ||
		checkReceipt.RequestSHA256 != authorization.AdapterCheckRequestSHA256 ||
		checkReceipt.AdapterCheckDirectoryHash != authorization.AdapterCheckDirectorySHA256 ||
		checkReceipt.ExpiresAt != authorization.AdapterCheckExpiresAt ||
		checkReceipt.FramingConformanceSHA256 != authorization.FramingConformanceSHA256 ||
		checkReceipt.ProtocolSHA256 != authorization.ProtocolSHA256 ||
		checkReceipt.ModelLoaded || checkReceipt.NetworkAllowed {
		return ExecutionAuthorization{}, HostSnapshot{}, ModelManifest{}, RuntimeManifest{}, nil, fmt.Errorf("authorization adapter-check receipt mismatch")
	}
	...
	if err := strictJSONObjectFileDecode(files["model-manifest.json"], &model); err != nil ||
	...
	if err := strictJSONObjectFileDecode(files["runtime-manifest.json"], &runtimeManifest); err != nil ||
...
func consumeAuthorization(directory string, authorization ExecutionAuthorization) (AttemptLedgerEntry, bool, error) {
	return consumeAuthorizationWithPostAppend(
		directory, authorization,
		func(file *os.File, path, directory string) error {
...
func consumeAuthorizationWithPostAppend(
	directory string,
	authorization ExecutionAuthorization,
	postAppend func(*os.File, string, string) error,
) (AttemptLedgerEntry, bool, error) {
	path := filepath.Join(directory, "attempt-ledger.jsonl")
	fd, err := unix.Open(path, unix.O_RDWR|unix.O_CLOEXEC|unix.O_NOFOLLOW, 0)
	if err != nil {
		return AttemptLedgerEntry{}, false, err
	}
	file := os.NewFile(uintptr(fd), path)
	if file == nil {
		_ = unix.Close(fd)
		return AttemptLedgerEntry{}, false, fmt.Errorf("open authorization attempt ledger")
	}
	defer func() { _ = file.Close() }()
	if err := unix.Flock(int(file.Fd()), unix.LOCK_EX|unix.LOCK_NB); err != nil {
		return AttemptLedgerEntry{}, false, fmt.Errorf("authorization attempt ledger is locked")
	}
	...
	if _, attempt, err := validateAttemptLedgerBytes(data, authorization, false); err != nil {
		...
		return AttemptLedgerEntry{}, false, fmt.Errorf("authorization has already been consumed")
	}
	...
	if err := postAppend(file, path, directory); err != nil {
...
func validateAttemptLedgerBytes(data []byte, authorization ExecutionAuthorization, requireAttempt bool) (AttemptLedgerGenesis, *AttemptLedgerEntry, error) {
	...
	if genesis.AdapterCheckReceiptSHA256 != authorization.AdapterCheckReceiptSHA256 {
		return AttemptLedgerGenesis{}, nil, fmt.Errorf("attempt ledger genesis mismatch")
	}
	if len(lines) == 1 {
		if requireAttempt {
	...
	if err := strictJSONPayloadDecode(lines[1], &attempt, true); err != nil {
		return AttemptLedgerGenesis{}, nil, err
	}
...
func validateExecutableBuildInfo(info *debug.BuildInfo, commit string) (executableBuild, error) {
	if info == nil || info.Path != "github.com/Siddhant-K-code/distill/cmd/distill-local-context-control-v2" {
