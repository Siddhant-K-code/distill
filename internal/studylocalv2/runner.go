...
func Run(ctx context.Context, options RunOptions) (completion CompletionRecord, runErr error) {
	...
	authorization, _, modelManifest, runtimeManifest, authorizationFiles, err := LoadAuthorization(options.AuthorizationDirectory)
	if err != nil {
		return CompletionRecord{}, err
	}
	if err := requireFreshAdapterCheck(authorization, time.Now().UTC()); err != nil {
		return CompletionRecord{}, err
	}
	if authorization.PackageManifestSHA256 != DigestBytes(pkg.Files["package-manifest.json"]) ||
		...
		authorization.ProtocolSHA256 != pkg.Protocol.ProtocolSHA256 {
	...
	adapter, err := StartCommandAdapter(ctx, CommandAdapterOptions{
		...
		PackageManifestPath:     filepath.Join(output, "package-manifest.json"),
		AuthorizationPath:       filepath.Join(output, "authorization.json"),
		AdapterCheckReceiptPath: filepath.Join(output, "adapter-check.json"),
		RuntimeTreeManifestPath: options.RuntimeTreeManifestPath,
		BaseTreeManifestPath:    options.BaseTreeManifestPath,
		WheelVerificationPath:   options.WheelVerificationPath,
		OutputNamespace:         filepath.Join(output, "adapter-workspace"),
		RunNamespace:            output,
		RunManifestPath:         filepath.Join(output, "run-manifest.json"),
	})
	if err != nil {
		return CompletionRecord{}, err
	}
	...
	preload := adapter.PreloadReady()
	if preload.PackageManifestSHA256 != authorization.PackageManifestSHA256 ||
		preload.AuthorizationSHA256 != authorization.AuthorizationSHA256 ||
		preload.AdapterCheckReceiptSHA256 != authorization.AdapterCheckReceiptSHA256 ||
		preload.ModelManifestSHA256 != modelManifest.ManifestSHA256 ||
		preload.RuntimeManifestSHA256 != runtimeManifest.ManifestSHA256 ||
		preload.OutputNamespaceSHA256 != authorization.OutputNamespaceSHA256 ||
		preload.ModelLoaded {
		return CompletionRecord{}, fmt.Errorf("adapter preload bindings mismatch")
	}
	if err := os.Mkdir(filepath.Join(output, "adapter-workspace"), 0o700); err != nil {
		return CompletionRecord{}, fmt.Errorf("create adapter workspace: %w", err)
	}
	if err := requireFreshAdapterCheck(authorization, time.Now().UTC()); err != nil {
		return CompletionRecord{}, fmt.Errorf("before attempt consumption: %w", err)
	}

	var appended bool
	attempt, appended, err = consumeAuthorization(options.AuthorizationDirectory, authorization)
	if appended {
	...
	if !appended {
		return CompletionRecord{}, fmt.Errorf("authorization consumption did not reach its durable commit point")
	}
	...
	if err := writeExclusive(filepath.Join(output, "run-manifest.json"), runManifestBytes, 0o600); err != nil {
		return CompletionRecord{}, err
	}
	loadContext, loadCancel := context.WithTimeout(ctx, 12*time.Minute)
	ready, err := adapter.Load(loadContext)
	loadCancel()
	if err != nil {
		return CompletionRecord{}, err
	}
...
func requireFreshAdapterCheck(authorization ExecutionAuthorization, now time.Time) error {
	if authorization.AdapterCheckExpiresAt.IsZero() || !now.Before(authorization.AdapterCheckExpiresAt) {
		return fmt.Errorf("adapter-check receipt expired")
	}
	return nil
}
