...
func WriteRuntimeManifest(ctx context.Context, options RuntimeManifestOptions) (RuntimeManifest, error) {
	...
	command := exec.CommandContext(
		...
		python, "-I", "-B", "-S", adapter, "runtime-manifest",
	...
	if err := strictJSONFrameDecode(stdout.Bytes(), &manifest); err != nil {
	...
	if err := writeExclusive(options.OutputPath, data, 0o600); err != nil {
		return RuntimeManifest{}, err
	}
	return manifest, nil
}
